...
<div class='flex items-center justify-center'>
	<div class="p-4 items-center justify-center w-[680px] rounded-xl group sm:flex space-x-6 bg-white bg-opacity-50 shadow-xl hover:rounded-2xl">
		<img  class="mx-auto w-full block w-4/12 h-40 rounded-lg"  alt="art cover" loading="lazy" src="../../src/assets/imagenes/<?=$rj["id"]?>.jpg"/>
		<div class="sm:w-8/12 pl-0 p-5">
          ...
          <h6 class="text-md font-semibold text-cyan-900 text-justify">
          <?=$rj["subcategoria"]?>
					</h6>
				</div>
				<div class="flex items-center space-x-4 justify-between">
					<div class="flex gap-3 space-y-1">
						<img  src="../../src/assets/imagenes/<?=$rj["id"]?>.jpg"  class="rounded-full h-8 w-8" />
						<span class="text-sm"><?=$rj["comentario"]?></span>
					</div>
					<div class=" px-3 py-1 rounded-lg flex space-x-2 flex-row">
						<div class="cursor-pointer text-center text-md justify-center items-center flex">
							...
							<span class="text-md mx-1"><?=$rj["valoracion"]?></span>
						</div>
					</div>
				</div>
				<div class="flex items-center space-x-4 justify-between">
        <div class="text-grey-500 flex flex-row space-x-1  my-4">
						...
						<p class="text-xs"><?=$rj["digital"]?></p>
					</div>
					<div class="text-grey-500 flex flex-row space-x-1  my-4">
            <p class="text-lg font-semibold text-neutral-700"><?=$rj["precio"]?>€</p>
					</div>
              ...
              <form method="post" action="../servicios/eliminarProducto.php?id=<?=$rj["id"]?> " >
                   <button  type="submit" value="Eliminar">
						...
						<form method="post" action="../servicios/editaReloj.php?id=<?=$rj["id"]?> " >
